...
export default function ProductClient({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (product?.variants && product.variants.length > 0 && !selectedVariantId) {
      const available = product.variants.find(v => v.stockQuantity > 0) || product.variants[0];
  ...
  const cartItem = cart?.items?.find((item) => item.productId === product?.id && item.variantId === (selectedVariantId || undefined));
  ...
  const handleAddToCart = useCallback(() => {
    ...
    addItem(product.id, 1, product, selectedVariantId || undefined);
  ...
  const currentPrice = selectedVariant?.priceInPesewas || product.priceInPesewas;
  const currentComparePrice = selectedVariant?.comparePriceInPesewas || product.comparePriceInPesewas;
  const currentStock = selectedVariant ? selectedVariant.stockQuantity > 0 : product.inStock;
  ...
  const AddToCartButton = ({ className }: { className?: string }) => {
    ...
    return (
      <button 
        ...
        disabled={!product.isActive || !currentStock}
      ...
      >
        ...
        {!product.isActive ? "Not Available" : currentStock ? "Add to Cart" : "Out of Stock"}
      </button>
    );
  };
            ...
            <div className="flex-1 p-6 lg:p-8 flex flex-col">
              <div className="flex-1">
                ...
                {product.variants && product.variants.length > 0 && (
                  <div className="mb-6">
                    <h3 className="text-sm font-semibold text-slate-900 mb-3">Available Options</h3>
                      ...
                      {product.variants.map((variant) => (
                        <button
                          ...
                          className={cn(
                            ...
                            variant.stockQuantity <= 0 && "opacity-50 cursor-not-allowed line-through hover:border-slate-200 text-slate-400"
                          )}
                        >
                          {variant.name}
                        </button>
                ...
                <div className="border-t border-b border-slate-100 py-6 mb-8">
                  <div className="flex items-baseline gap-3 mb-2">
                    <span className="text-3xl lg:text-4xl font-bold text-slate-900">₵{(currentPrice / 100).toLocaleString()}</span>
                    {currentComparePrice && (
                      <span className="text-lg text-slate-400 line-through">₵{(currentComparePrice / 100).toLocaleString()}</span>
                    )}
                  </div>
                  {currentComparePrice && (
                    <p className="text-sm text-orange-600 font-medium">
                      You save ₵{((currentComparePrice - currentPrice) / 100).toLocaleString()}
                    </p>
                  )}
