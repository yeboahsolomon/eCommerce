...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const data = await paymentService.initializeMomoPayment(orderId, phoneNumber, userId);
      res.json({ success: true, message: 'Approve the payment on your phone', data });
    } catch (error: any) {
      if (error.message === 'Order not found or already paid' || error.message === 'Payment already in progress') {
        res.status(error.message === 'Order not found or already paid' ? 404 : 400).json({ success: false, message: error.message });
        return;
      }
      next(error);
    }
...
router.get('/momo/verify/:reference', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const data = await paymentService.initializePaystackPayment(orderId, email, callbackUrl, paymentMethod, mobileMoneyProvider, mobileMoneyNumber, userId);
      res.json({ success: true, data });
    } catch (error: any) {
      if (['Order not found', 'Payment already in progress', 'Mobile money provider and phone number are required for MoMo payments'].includes(error.message)) {
        res.status(error.message === 'Order not found' ? 404 : 400).json({ success: false, message: error.message });
        return;
      }
      next(error);
    }
...
router.get('/paystack/verify/:reference', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { reference } = req.params;
    const userId = (req as any).user.id;

    const data = await paymentService.verifyPaystackPayment(reference, userId);
    res.json({ success: data.status !== 'FAILED', data });
  } catch (error: any) {
    if (error.message === 'Payment not found') {
      res.status(404).json({ success: false, message: error.message });
...
router.post('/:paymentId/cancel', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/refund/:orderId', authMiddleware, validate(refundSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { orderId } = req.params;
    ...
    const data = await paymentService.processRefund(orderId, reason, amountInPesewas, user.id);
    res.json({ success: true, message: 'Refund processed successfully', data });
  } catch (error: any) {
    if (error.message === 'Order not found' || error.message === 'No successful payment to refund' || error.message.startsWith('Maximum refundable')) {
      res.status(error.message === 'Order not found' ? 404 : 400).json({ success: false, message: error.message });
      return;
    }
...
router.get('/refunds/:orderId', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { orderId } = req.params;
    const user = (req as any).user;
...
router.get('/refunds', authMiddleware, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const user = (req as any).user;
    if (user.role !== 'ADMIN') {
      return res.status(403).json({ success: false, message: 'Admin access required' });
