...
export class PaymentService {
  ...
  async initializeMomoPayment(orderId: string, phoneNumber: string, userId: string) {
    ...
    return { paymentId: payment.id, reference: momoResponse.referenceId, amount: order.totalInPesewas / 100, provider: 'mtn_momo' };
  }
  ...
  async initializePaystackPayment(orderId: string, email: string, callbackUrl: string | undefined, paymentMethod: string, mobileMoneyProvider: string | undefined, mobileMoneyNumber: string | undefined, userId: string) {
    const order = await prisma.order.findFirst({
      where: { id: orderId, userId, status: { in: ['PENDING', 'PAYMENT_PENDING'] } },
      include: { payment: true },
    });
    ...
    if (paymentMethod === 'mobile_money' && (!mobileMoneyProvider || !mobileMoneyNumber)) {
      throw new Error('Mobile money provider and phone number are required for MoMo payments');
    }
    ...
    const paystackRequest: any = {
      ...
      metadata: {
        ...
        userId,
        paymentMethod,
        custom_fields: [{ display_name: 'Order Number', variable_name: 'order_number', value: order.orderNumber }],
      },
    };
  ...
  async verifyPaystackPayment(reference: string, userId: string) {
    ...
    if (paystackResponse.data.status === 'success') {
      ...
      if (paystackResponse.data.channel === 'mobile_money') method = 'MOMO_MTN' as any;
      ...
      return { status: 'SUCCESS', orderNumber: payment.order.orderNumber, channel: paystackResponse.data.channel, cardLast4: paystackResponse.data.authorization?.last4 };
      ...
      return { status: 'FAILED', message: paystackResponse.data.gateway_response };
    }

    return { status: paystackResponse.data.status.toUpperCase(), orderNumber: payment.order.orderNumber };
  }

  async handlePaystackWebhook(event: any) {
    if (event.event === 'charge.success') {
      const { reference } = event.data;
      const payment = await prisma.payment.findFirst({ where: { gatewayReference: reference }, include: { order: true } });
      ...
      await prisma.$transaction([
        prisma.payment.update({
          ...
          data: {
            status: 'SUCCESS',
            confirmedAt: event.data.paid_at ? new Date(event.data.paid_at) : new Date(),
            method: method as any,
      ...
      await this.sendPaymentConfirmationEmail(payment.orderId).catch(console.error);
    } else if (event.event === 'charge.failed') {
      const { reference } = event.data;
      const payment = await prisma.payment.findFirst({ where: { gatewayReference: reference } });
      if (!payment || payment.status === 'FAILED') return;
      await prisma.$transaction([
        prisma.payment.update({
          where: { id: payment.id },
          data: { status: 'FAILED', failedAt: new Date(), failureReason: event.data.gateway_response || 'Payment failed', gatewayResponse: event.data },
        }),
        prisma.order.update({ where: { id: payment.orderId }, data: { status: 'FAILED' } }),
      ]);
    } else if (event.event === 'transfer.success') {
      const { reference, transfer_code } = event.data;
      const payout = await prisma.payout.findFirst({ where: { OR: [{ transactionReference: reference }, { transactionReference: transfer_code }] } });
      ...
      const payout = await prisma.payout.findFirst({ where: { OR: [{ transactionReference: reference }, { transactionReference: transfer_code }] }, include: { seller: { include: { wallet: true } } } });
      if (!payout || payout.status === 'FAILED') return;
      if (payout.seller?.wallet) {
        await prisma.$transaction([
          prisma.payout.update({ where: { id: payout.id }, data: { status: 'FAILED', notes: reason || 'Transfer failed' } }),
          prisma.sellerWallet.update({ where: { id: payout.seller.wallet.id }, data: { currentBalance: { increment: payout.amount }, totalWithdrawn: { decrement: payout.amount } } }),
          prisma.transaction.create({ data: { walletId: payout.seller.wallet.id, type: 'ADJUSTMENT', amount: payout.amount, balanceBefore: payout.seller.wallet.currentBalance, balanceAfter: payout.seller.wallet.currentBalance + payout.amount, description: `Payout refund — transfer failed: ${reason || 'Unknown'}`, referenceId: payout.id } }),
        ]);
      }
    }
  }
  ...
  async processRefund(orderId: string, reason: string, amountInPesewas: number | undefined, adminUserId: string) {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      include: {
        payment: true,
    ...
    if (!order.payment || order.payment.status !== 'SUCCESS') throw new Error('No successful payment to refund');
    ...
    if (refundAmount > maxRefundable) throw new Error(`Maximum refundable amount is ${(maxRefundable / 100).toFixed(2)}`);

    const refund = await prisma.refund.create({ data: { paymentId: order.payment.id, amountInPesewas: refundAmount, reason, status: 'PENDING' } });

    let gatewaySuccess = false;
    if (order.payment.gatewayReference && order.payment.gatewayProvider === 'paystack') {
      gatewaySuccess = await paystackService.createRefund(order.payment.gatewayReference, refundAmount);
    } else {
      gatewaySuccess = true;
    }
    ...
    if (gatewaySuccess) {
      await prisma.$transaction(async (tx) => {
        await tx.refund.update({ where: { id: refund.id }, data: { status: 'SUCCESS', processedAt: new Date(), gatewayReference: order.payment?.gatewayReference || null } });
        if (refundAmount >= order.payment!.amountInPesewas) {
          await tx.order.update({ where: { id: orderId }, data: { status: 'REFUNDED' } });
          await tx.sellerOrder.updateMany({ where: { orderId }, data: { status: 'REFUNDED' } });
        }
        for (const sellerOrder of order.sellerOrders) {
          if (!sellerOrder.seller?.wallet) continue;
          const sellerRefundPortion = Math.round((refundAmount * sellerOrder.payoutAmountInPesewas) / order.totalInPesewas);
          if (sellerRefundPortion > 0) {
            const wallet = sellerOrder.seller.wallet;
            await tx.sellerWallet.update({ where: { id: wallet.id }, data: { pendingBalance: { decrement: Math.min(sellerRefundPortion, wallet.pendingBalance) }, totalEarned: { decrement: sellerRefundPortion } } });
            await tx.transaction.create({ data: { walletId: wallet.id, type: 'REFUND', amount: -sellerRefundPortion, balanceBefore: wallet.pendingBalance, balanceAfter: wallet.pendingBalance - sellerRefundPortion, description: `Refund for order #${order.orderNumber}: ${reason}`, referenceId: refund.id } });
          }
        }
        if (refundAmount >= order.payment!.amountInPesewas) {
          for (const item of order.items) {
            ...
            if (product?.trackInventory) {
              ...
              await tx.inventoryLog.create({ data: { productId: item.productId, action: 'RETURN', quantityChange: item.quantity, previousQuantity: product.stockQuantity, newQuantity: product.stockQuantity + item.quantity, orderId, userId: adminUserId, notes: `Refund: ${reason}` } });
            }
          }
        }
      });
      return { refundId: refund.id, amountInCedis: refundAmount / 100, status: 'SUCCESS' };
    } else {
      await prisma.refund.update({ where: { id: refund.id }, data: { status: 'FAILED' } });
      throw new Error('Refund processing failed at payment gateway. Please try again.');
    }
  }
  ...
  async getRefundsForOrder(orderId: string, user: any) {
    ...
    const refunds = await prisma.refund.findMany({ where: { paymentId: order.payment.id }, orderBy: { createdAt: 'desc' } });
    return refunds.map((r) => ({ id: r.id, amountInCedis: r.amountInPesewas / 100, reason: r.reason, status: r.status, processedAt: r.processedAt, createdAt: r.createdAt }));
  }

  async getAllRefunds(page: number, limit: number) {
    const [refunds, total] = await Promise.all([
      prisma.refund.findMany({
        include: { payment: { select: { orderId: true, method: true, order: { select: { orderNumber: true, customerEmail: true } } } } },
    ...
    return {
      refunds: refunds.map((r) => ({ id: r.id, orderNumber: r.payment.order.orderNumber, customerEmail: r.payment.order.customerEmail, amountInCedis: r.amountInPesewas / 100, reason: r.reason, status: r.status, paymentMethod: r.payment.method, processedAt: r.processedAt, createdAt: r.createdAt })),
  ...
  async getPaymentForOrder(orderId: string, userId: string) {
    ...
    return {
      ...
      cardLast4: payment.cardLast4, cardBrand: payment.cardBrand, momoNetwork: payment.momoNetwork, momoPhoneNumber: payment.momoPhoneNumber,
  ...
  async getPaymentHistory(userId: string, page: number, limit: number) {
    const [payments, total] = await Promise.all([
      prisma.payment.findMany({
        where: { order: { userId } },
        include: { order: { select: { orderNumber: true, status: true, totalInPesewas: true, createdAt: true } } },
        orderBy: { createdAt: 'desc' },
        take: limit,
        skip: (page - 1) * limit,
      }),
    ...
    return {
      payments: payments.map((p) => ({
        id: p.id, orderNumber: p.order.orderNumber, amount: p.amountInPesewas / 100, method: p.method, provider: p.gatewayProvider,
        status: p.status, reference: p.gatewayReference, cardLast4: p.cardLast4, momoNetwork: p.momoNetwork,
        confirmedAt: p.confirmedAt, createdAt: p.createdAt,
      })),
      pagination: { page, limit, total, pages: Math.ceil(total / limit) },
    };
  }
