...
export class ProductService {
  async getProducts(query: any) {
    const { page = '1', limit = '20', category, search, inStock, minPrice, maxPrice, sortBy = 'createdAt', order = 'desc', featured, seller } = query;
    ...
    if (inStock === 'true') {
      where.OR = [{ trackInventory: false }, { stockQuantity: { gt: 0 } }, { allowBackorder: true }];
    }
    ...
    const [products, total] = await Promise.all([
      prisma.product.findMany({
        ...
        include: {
          ...
          variants: true,
          seller: { select: { id: true, businessName: true, slug: true, ghanaRegion: true, businessAddress: true, logoUrl: true } },
        },
        orderBy,
        skip,
        take: limitNum,
  ...
  async getProductById(id: string, user: any) {
    ...
    if (!product) {
      product = await prisma.product.findFirst({
        ...
        include: {
          category: true,
          seller: { select: { id: true, businessName: true, slug: true, logoUrl: true, ghanaRegion: true, businessAddress: true } },
    ...
    prisma.product.update({ where: { id: product.id }, data: { views: { increment: 1 } } }).catch(console.error);
  ...
  async getRelatedProducts(id: string) {
    ...
    const relatedProducts = await prisma.product.findMany({
      where: { id: { not: product.id }, isActive: true, OR: [{ categoryId: product.categoryId }] },
  ...
  async createProduct(data: CreateProductInput & { images?: string[]; variants?: any[] }, user: any) {
    const { images, variants, ...productData } = data;

    let sellerId = productData.sellerId;
    if (user?.role === 'SELLER') {
      if (!user.sellerProfile) throw new ApiError(400, 'Seller profile not found. Please complete your seller profile.');
    ...
    const product = await prisma.product.create({
      data: {
        ...
        slug,
        sellerId,
        images: images && images.length > 0 ? { create: images.map((url, index) => ({ url, isPrimary: index === 0, sortOrder: index })) } : undefined,
  ...
  async updateProductImages(id: string, images: any[], user: any) {
    ...
    if (user.role !== 'ADMIN') {
      if (existingProduct.sellerId !== user.sellerProfile?.id) throw new ApiError(403, 'Not authorized to update this product');
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (images.length > 0) {
        await tx.productImage.createMany({
          data: images.map((img: any, index: number) => ({ productId: id, url: img.url, isPrimary: img.isPrimary || index === 0, sortOrder: img.sortOrder || index })),
        });
      }
    });
    ...
    const updatedProduct = await prisma.product.findUnique({ where: { id }, include: { images: { orderBy: { sortOrder: 'asc' } } } });
    return { images: updatedProduct?.images };
  }

  async updateProduct(id: string, data: UpdateProductInput & { images?: string[]; variants?: any[] }, user: any) {
    ...
    if (user?.role === 'SELLER') {
      if (!user.sellerProfile) throw new ApiError(403, 'Seller profile not found.');
      if (existingProduct.sellerId !== user.sellerProfile.id) throw new ApiError(403, 'You can only update your own products.');
    }
    ...
    if (updateData.sku && updateData.sku !== existingProduct.sku) {
      const existingSku = await prisma.product.findFirst({ where: { sku: updateData.sku, id: { not: id } } });
      if (existingSku) throw new ApiError(409, 'A product with this SKU already exists.');
    }
    ...
    if (updateData.stockQuantity !== undefined && updateData.stockQuantity !== existingProduct.stockQuantity) {
      ...
      await prisma.inventoryLog.create({
        data: { productId: id, action, quantityChange, previousQuantity: existingProduct.stockQuantity, newQuantity: updateData.stockQuantity, userId: user?.id, notes: 'Manual update via Product Edit' },
      });
    }
    ...
    if (images !== undefined) {
      ...
      if (images.length > 0) {
        await prisma.productImage.createMany({
          data: images.map((url, index) => ({ productId: id, url, isPrimary: index === 0, sortOrder: index })),
        });
      }
    }
    ...
    if (variants !== undefined) {
      ...
      if (variants.length > 0) {
        await prisma.productVariant.createMany({ data: variants.map((v: any) => ({ productId: id, ...v } as any)) });
  ...
  async deleteProduct(id: string, user: any) {
    ...
    if (user?.role === 'SELLER') {
      if (!user.sellerProfile) throw new ApiError(403, 'Seller profile not found.');
      if (product.sellerId !== user.sellerProfile.id) throw new ApiError(403, 'You can only delete your own products.');
    }
    ...
    if (orderCount > 0) {
      ...
      await redisService.del(`products:detail:${product.slug}`);
      await redisService.clearPattern('products:list:*');
      return { success: true, message: 'Product deactivated (preserved for order history).' };
    } else {
